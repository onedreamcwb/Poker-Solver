...
import { lateStealsRanges } from './ranges/late_steals.js'; // Novo import
...
export function evaluateAction(mode, heroPos, villainPos, handNotation, action) {
    switch (mode) {
        ...
        case 'sb-vs-rfi': return evaluateLateSteals('sb', villainPos, handNotation, action); // Novo
        case 'btn-vs-rfi': return evaluateLateSteals('btn', villainPos, handNotation, action); // Novo
        case 'cbet-ip': return { correct: false, msg: `Módulo C-Bet IP em construção.` };
...
function evaluateRFI(position, handNotation, action) {
    ...
    if (action === 'raise') {
        ...
        return !isRaiseHand ? { correct: true, msg: `Bom Fold. ${handNotation} não joga de ${position.toUpperCase()}.` } : { correct: false, msg: `Erro crítico! ${handNotation} é forte demais para foldar. Faça RAISE.` };
    } else if (action === 'call') {
        return { correct: false, msg: `Erro! Evite Open Limp. RAISE ou FOLD.` };
    }
}
...
function evaluateBBvsRFI(villainPos, handNotation, action) {
    ...
    if (action === '3-bet') {
        ...
        if (shouldCall) return { correct: false, msg: `Excesso de agressividade. ${handNotation} joga melhor de CALL.` };
        return { correct: false, msg: `Erro! ${handNotation} é fraco. FOLD.` };
    } else if (action === 'call') {
